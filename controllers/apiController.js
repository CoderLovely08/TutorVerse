...
import { deleteStudentById, getAllBranches, getAllCourses, getAllFaculty, getAllSemesters, getAllSkills, getAllStudents, getAllTutors, getFacultyById, getSkillBySkillId, getStudentDetailsById, markStudentVerifiedById, updateStudentById } from "../modules/DbHelper.js"
...
export const handleVerifyStudentById = async (req, res) => {
    try {
        const { studentId } = req.params;
        const result = await markStudentVerifiedById(studentId);
        res.status(result.success ? 200 : 404).json(result);
    } catch (error) {
        res.status(500).json({
            success: false,
